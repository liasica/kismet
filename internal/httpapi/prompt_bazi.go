...
// baziSystemPrompt 解读的角色与批命规则
const baziSystemPrompt = `你是执业多年的八字命理师，按子平法批命。批命的顺序是：先看月令与日主定旺衰，再定格局与用神喜忌，然后由用神出发论性情、六亲、事业、财运、婚姻、健康，最后看大运流年与原局的作用。与用神无关的泛论不写。全文用简体中文，像面对面给客人批命那样直接、具体，不客套。
...
// baziAdultSections 成人的章节清单，两个占位符依次为当前与下一个流年
const baziAdultSections = `请按下面的章节批命，总篇幅 3500 到 4500 字：
## 命局总论
...
五行偏枯与刑冲对应的脏腑方向与日常注意。
## 大运与流年
当前大运的主题；%[1]s、%[2]s各自在事业、财运、感情上的要点；这步大运里的关键年份与理由。
## 建议
这一节 1000 到 1400 字，按下面三个三级标题依次写。每条都落到能做的动作上，写明由哪个用神、喜神、忌神、刑冲或哪步岁运推出，与前面各节的判断一一对应；命盘支持几条写几条，推不出来的项略过，不要为凑齐条目编。
### 开运
把用神喜神落到日常：宜用的颜色与穿着，宜居宜业的方位与座向、宜往的发展方向，有利的数字与随身的材质器物，宜近的行业与做事方式（宜自主还是宜依附、宜动还是宜静），该亲近的贵人是哪类人（用神所属十神对应的长辈、同辈、异性或下属），一年里有利的月份与一天里有利的时辰，顺应用神的作息、饮食与运动。
### 避坑
把忌神与刑冲落到日常：该戒的习惯与该收敛的心性，要避开的行业与合作方式，破财的口子（合伙、担保、借贷、投机、置产，按命盘实际配置点名），感情上最容易踩的坑，健康上的红线，口舌与文书契约上要防的事，不利的月份与要绷紧的年份。
### 近两年怎么做
%[1]s 与 %[2]s 各给三到五条具体动作，说清楚做什么、避什么、什么时候动手。`

// baziMinorSections 未成年人的章节清单，两个占位符依次为当前与下一个流年
const baziMinorSections = `命主是未成年人，批命对象是其父母，不谈婚姻、财运与事业。请按下面的章节批命，总篇幅 2600 到 3400 字：
## 命局总论
...
## 大运与流年
当前所处的大运或起运前阶段的主题；%[1]s、%[2]s孩子的状态与父母要留意的事。
## 给父母的建议
这一节 900 到 1200 字，按下面三个三级标题依次写。每条都落到父母能直接执行的动作上，写明由哪个用神、喜神、忌神或哪种刑冲推出，与前面各节的判断一一对应；命盘支持几条写几条，推不出来的项略过，不要为凑齐条目编。
### 开运
把用神喜神落到孩子的日常：宜用的颜色，居室与书桌的方位座向，宜近的学习方向与才艺，适合的运动、作息与饮食，该多接触的长辈或老师是哪类人，一年里状态较好的月份。
### 避坑
把忌神与刑冲落到孩子的日常：管教上最容易出问题的地方，该限制的习惯与环境，体质上的红线与换季注意，人际与安全上要留心的方向，不利的月份与要多看顾的年份。
### 近两年怎么做
%[1]s 与 %[2]s 各给三到五条父母能直接执行的动作。`

// baziAnalysisMessages 由排盘结果与当前时刻拼出发给模型的消息
func baziAnalysisMessages(chart bazi.Chart, now time.Time) ([]chatMessage, error) {
	user, err := baziUserPrompt(chart, now.In(beijingZone))
	if err != nil {
		return nil, err
	}
	return []chatMessage{
...
// baziUserPrompt 命主信息、今天所处的大运流年、文字命盘与章节清单
func baziUserPrompt(chart bazi.Chart, now time.Time) (string, error) {
	...
	b.WriteString(baziDecadeClause(chart, position))
	b.WriteString("。\n\n以下是排盘软件输出的命盘：\n\n<命盘>\n")
	b.WriteString(bazi.ToText(chart, bazi.ToTextOptions{Decades: true, Years: true}))
	b.WriteString("\n</命盘>\n\n")

	sections := baziAdultSections
	if position.Age <= minorMaxAge {
		sections = baziMinorSections
	}
	_, _ = fmt.Fprintf(&b, sections, thisYear, nextYear)
	return b.String(), nil
}

// baziDecadeClause 所处的大运，起运之前改为说明起运时刻
func baziDecadeClause(chart bazi.Chart, position bazi.FortunePosition) string {
